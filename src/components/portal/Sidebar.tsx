  ...
  return (
    <aside className="w-72 h-screen bg-cream-50/90 backdrop-blur-md border-r border-lavender-200/60 flex flex-col fixed left-0 top-0 z-50 shadow-soft">
      ...
      <nav className="flex-1 overflow-y-auto p-4">
        <h2 className="text-xs font-semibold text-charcoal-400 uppercase tracking-wider mb-3 px-2">
          Your Sessions
        </h2>
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <span className={`font-medium truncate text-sm
                                       ${isActive ? 'text-lavender-700' : 'text-charcoal-700'}`}>
                        {project.child_name || project.title}
                      </span>
                    </div>
                    {project.occasion && (
                      <p className="text-xs text-charcoal-400 capitalize mb-2 truncate">
                        {project.occasion}
                      </p>
                    )}
      ...
      {/* User Menu */}
      <div className="p-4 border-t border-lavender-100 bg-cream-100/50">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-10 h-10 rounded-full bg-lavender-200 flex items-center justify-center">
            <span className="text-lavender-700 font-semibold text-sm">
        ...
        <button
          ...
          className="w-full px-4 py-2 text-sm text-charcoal-500 hover:text-lavender-700
                     hover:bg-lavender-50 rounded-lg transition-all duration-[400ms]
