...
export default function PricingCards() {
  return (
    <section id="pricing" className="py-20 px-6 bg-gradient-to-b from-cream-50 to-sage-50">
      <div className="max-w-7xl mx-auto">
        {/* Section Header */}
        <div className="text-center mb-16">
          <p className="font-serif text-xl text-sage-500 mb-2">Investment</p>
          ...
          {pricingPackages.map((pkg, index) => (
            <div
              ...
              className={`relative card flex flex-col transition-all duration-400 hover:shadow-lifted hover:-translate-y-1 ${
                pkg.popular
                  ? 'border-2 border-sage-400 shadow-soft'
            ...
            >
              ...
              {pkg.popular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                  <span className="bg-sage-500 text-white text-sm font-medium px-4 py-1 rounded-full shadow-soft">
                    Most Popular
                  </span>
                </div>
              )}
              ...
              <div className="p-8 flex flex-col flex-grow">
                ...
                {/* Price */}
                <div className="mb-6">
                  <span className="font-serif text-5xl text-sage-600">{pkg.price}</span>
                </div>

                {/* Photo Count Highlight */}
                <div className="bg-sage-100 rounded-2xl p-4 mb-6 text-center">
                  <p className="font-serif text-3xl text-sage-700">{pkg.photos}</p>
                  <p className="text-sage-600 text-sm font-medium">Digital Photos</p>
                </div>

                {/* Features List */}
                <ul className="space-y-3 mb-8 flex-grow">
                  {pkg.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3 text-sage-600">
                      <svg
        ...
        {/* Additional Note */}
        <p className="text-center text-sage-400 text-sm mt-12">
